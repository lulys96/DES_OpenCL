...
public:
	shift_RL2(Port< bool , 100000>& _input1$FIFO,Port< bool , 100000>&  _output$FIFO) : input1$FIFO(_input1$FIFO),output$FIFO(_output$FIFO) {
			SetupOpenCL(&ocl, CL_DEVICE_TYPE_CPU, "Intel(R) OpenCL");
			CreateAndBuildProgram(&ocl, "shift_RL2.cl");
			cl_int err = CL_SUCCESS;
			ocl.kernels.push_back(clCreateKernel(ocl.program, "action$62", &err));
			if(err != CL_SUCCESS) {
				std::cout << "Kernel Creation failed. clCreateKernel for action$62  in  shift_RL2 returned " << TranslateErrorCode(err);
				exit(err);
			}
	}
