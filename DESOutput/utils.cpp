...
cl_platform_id FindOpenCLPlatform(cl_device_type deviceType,std::string platformName) {
	...
	for (cl_uint i = 0; i < numPlatforms; ++i) {
		char platformName[64];  //added code
		bool platformFound = false;
		cl_uint numDevices = 0;
		size_t stringLength = 0;
		//errorCode = clGetPlatformInfo(platforms[i], CL_PLATFORM_NAME, 0, NULL, &stringLength);
		errorCode = clGetPlatformInfo(platforms[i], CL_PLATFORM_NAME, 64, platformName, &stringLength);
		if (errorCode != CL_SUCCESS) {
			...
			if (errorCode != CL_SUCCESS) {
				...
				if (strstr(&foundPlatformName[0], platformName) != 0) { //added code
					platformFound = true;
...
cl_int CreateAndBuildProgram(opencl_arguments *ocl, std::string fileName) {
	...
	if (errorCode == CL_BUILD_PROGRAM_FAILURE) {
		...
		char *logE = (char *)malloc(log_size);

		clGetProgramBuildInfo(ocl->program, ocl->device, CL_PROGRAM_BUILD_LOG, log_size, logE, NULL);

		std::cout << logE << std::endl;
